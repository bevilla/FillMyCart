...
namespace FillMyCart
{
    ...
    class ProductViewerStartPatch
    {
        ...
        static void Postfix(ProductViewer __instance)
        {
            ...
            var productTypeDropdown = AccessTools.Field(typeof(ProductViewer), "m_ProductTypeDropdown").GetValue(__instance) as TMP_Dropdown;
            ...
            var image = autoFillButton.AddComponent<Image>();
            image.color = new Color(0.0f, 0.77f, 0.02f);

            var button = autoFillButton.AddComponent<Button>();
            button.onClick.AddListener(() => Plugin.AutoFill());

            var rect = autoFillButton.GetComponent<RectTransform>();
